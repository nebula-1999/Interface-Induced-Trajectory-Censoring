...
def load_bfcl_scores(root):
    """Return model -> case -> BFCL validity; skip each score file's header row."""
    out = defaultdict(dict)
    if not root:
        return out
    root = Path(root)
    for path in root.rglob("BFCL_v4_*_score.json"):
        ...
        for line in path.open(encoding="utf-8"):
            if line.strip():
                row = json.loads(line)
                if "id" in row and "valid" in row:
                    out[model][row["id"]] = bool(row["valid"])
    return out
...
def main():
    ...
    print_joined_table(by, load_bfcl_results(args.result_root), load_bfcl_scores(args.score_root))
    return 0
