...
> **⚠ 2026-09-03 19:40 修正**：本文件原有的分支 A / 分支 B 框架
> **按本次 run 的数据无法触发**——两臂都跑在 `test_freq=-1 / save_freq=-1` 下，
> §5.9 sufficiency 主张所依据的「保留集救回数」没有被测量，事后也补不回来。
> 用 `critic/score/mean` 判分支会重蹈本文第 1 节批评的那个混淆。
> **实际按分支 C 走**，见 `p3/PREREG_AMENDMENT_20260903.md` 第五节。
> 该修正在看到 repaired 任何结果之前提交。
