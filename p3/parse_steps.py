...
**判据已作废,不要用本脚本判分支 A/B。** 见 p3/PREREG_AMENDMENT_20260903.md。

原判据是「tool_calls_time > 0 且 **critic/score/mean 上升** → 分支 A」。那是错的:
`critic/score/mean` 是训练侧聚合奖励,而 §5.9 的 sufficiency 主张关于的是
**保留集上的多轮救回数**。本文第 1 节的全部意义就是这两个量不同——历史 run 里
pass@1 涨 2.8 分而救回数纹丝不动,91–94% 的新通过题是首轮过的。
拿聚合分判分支,等于用本文自证为混淆的量去推翻本文的结论。

**正确判据**:读评测钩子落盘的 runs/code-eval*/step_NNNNN.jsonl,按
analyze_step3.py 的口径算 rescued = (最终过 且 首轮没过),看它随 step 的走势。
2026-09-15 更正：历史 multi 的1108行含重复，不能作为分母，连比率与趋势也
不能直接引用。只读取具备唯一身份、固定manifest及complete校验的重新评测。
详见 ACCEPTANCE_20260915.md；此脚本不提供学习效果或分支判定。

本脚本保留的用途:抽取每步的机制指标(tool_calls_time、num_turns、response_length),
用来确认「通道是否打开」——那个它是可靠的。
