...
**Tool-calling serving stacks.** vLLM's tool-calling documentation specifies a distinct
required configuration per family: `llama3_json` plus a family chat template for Llama 3.1;
`mistral` plus `--tokenizer-mode hf --config-format hf --load-format hf` and a parallel
template for HF-format Mistral; `hermes` for Qwen2.5. It also documents that under
`tool_choice="auto"`, schema-level constraint applies only when `VLLM_ENFORCE_STRICT_TOOL_CALLING`
is set *and* at least one tool declares `strict: true`; otherwise "arguments may
occasionally be malformed or violate the function's parameter schema." Section 5.3 shows
this sentence describes a 23% failure mode we initially misattributed to the model.

**Known model-specific issues.** vLLM documents that parallel tool calls are unsupported
for Llama 3, that "Llama's smaller models frequently fail to emit tool calls in the correct
format," and that "Mistral 7B struggles to generate parallel tool calls correctly." Our
results are consistent with these and quantify their downstream cost.

**Prior report of the Qwen case.** hanXen (vLLM issue #32926, 2026-01-23) documents that
Qwen2.5-Coder was not trained on tool-call tokens, emits ```json blocks or bare JSON
without format instructions, and reaches 98–100% compliance with a `<tools>` few-shot
template plus a dedicated parser. Our §5.2 reproduces this baseline observation exactly
(zero `<tools>`/`<tool_call>` tags, all JSON) and extends it to scale, to other families,
and to training.

**Agent evaluation.** Reported tool-call rates are near-universally taken from the serving
layer's parsed output. We are not aware of prior work that separates emitted-but-unparsed
calls as a distinct measured quantity, nor of work connecting interface-layer censoring to
RL trajectory composition.
