...
under different rules (\S\ref{sec:scale}). Three things separate the contributions. Their gap is a single
...
four model families (\S\ref{sec:families}). Their gap is a fixed quantity; ours is a function of scale, and it
grows (\S\ref{sec:scale}). And their distortion terminates at the reported number, whereas we follow it
into an RL rollout and identify which branch of the policy the gradient can no longer reach
(\S\ref{sec:rl}). Outside the literature, the claim that tool-call failures originate in serving
...
directly on \S\ref{sec:llama}, where enabling \texttt{strict:\ true} eliminates a 23\% role-confusion failure and
...
--- outweighs any reasoning cost, and we report the CoT control (\S\ref{sec:llama}) where added reasoning
...
knowledge these report tool-call rates as parsed by the serving layer. \S\ref{sec:scale} shows that
...
counterpart: $\tau^2$-Bench-Verified~\citep{tau2benchverified}, a release of the same benchmark
...
layers of \S\ref{sec:families} are a quantified instance of a single harness-side edge in its schema.
...
preflight check of \S\ref{sec:limitations} does in seconds. And the most recent synthesis of the field's failure
...
per-family configuration, which \S\ref{sec:families} shows is not optional. verl~\citep{sheng2025hybridflow} provides the RL training stack; importantly its AgentLoop performs \textbf{its own}
...
one wasted experiment design (\S\ref{sec:rl}).
