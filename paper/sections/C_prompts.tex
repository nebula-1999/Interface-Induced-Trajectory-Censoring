...
comparison in \S5.6 uses \texttt{mandatory} on both sides, so ``FC executed zero tool calls''
...
\texttt{rich} is the \S5.3 control. Its purpose is to test whether the wrong-tool rate is
...
and they are the same parameter names that failed under \texttt{terse} (\S5.3, $p$=1.000).
...
\subsection{The Thought-scaffold control (\S5.3)}
...
\subsection{The role-disambiguation few-shot (\S5.6)}
