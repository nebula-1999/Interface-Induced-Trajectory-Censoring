...
under four rules excludes ``your extractor is simply better than \texttt{hermes}'': the hermes
column is zero at every scale because the model never produces that format, while the bare-JSON
and tight columns are not. Replaying vLLM's extractor line for line to locate the layer at which
each item is lost shows \textbf{genuine parser loss is zero in every arm}, and across all 4254
de-duplicated first turns in the archive; the one place strictness alone costs anything is
Instruct-3B, where 26 payloads are valid JSON followed by trailing text.\footnote{The 1.5B row uses
the \emph{optional} tool instruction. Under the \emph{mandatory} prompt used for training
(\S\ref{sec:rl}) the same checkpoint gives weak = 66 and unparsable = 46: coercion produces more
JSON-shaped debris, not more valid calls. The two numbers describe different prompts, not a
discrepancy.}

\paragraph{The counterfactual.} The server column is flat zero, so the growth above comes entirely
from the emitted column --- and larger checkpoints emit more well-formed JSON for reasons
unrelated to tool calling. Qwen3 supplies a ladder whose template injects tools and requires the
\texttt{<tool\_call>} envelope \texttt{hermes} accepts; we committed the prediction (``parsed $>0$
at every size'') to the repository before running (\texttt{p4/PREREGISTRATION.md}). Server-parsed
counts are 86 / 44 / 70 / 97 at 0.6B--8B, and the silent fraction is \textbf{1, 1, 2, 0} against
the mismatched ladder's 0, 4, 21, 36, 80 (Table~\ref{tab:counterfactual}). \textbf{Across a
comparable span of scale under a matched envelope, the silent fraction does not grow.} This rules
out capability growth as a \emph{sufficient} cause; it does not isolate the envelope, since the
ladder changes family and envelope together. Two caveats we state rather than bury: the parsed
column is \textbf{not monotone}, so this ladder shows presence, not a trend; and Qwen3 was served
with \texttt{enable\_thinking=false}, because with thinking on the model spends its whole token
budget in a \texttt{<think>} block and is truncated before emitting a call --- a confound unrelated
to the envelope, recorded as a protocol deviation before the run.

A second family exhibiting the \emph{same} undercount would be stronger still, and we did not get
one. Of nine families surveyed beforehand, six do not inject OpenAI-style tools at all, and of the
remaining three Granite-3.1-8B turned out not to attempt tool calls on this task: under
\texttt{hermes} and under its own \texttt{granite} parser both give 0, and \textbf{the rescue arm
is what tells us the arm is uninformative rather than confirmatory}. We claim nothing from it.
...
\emph{not} at the sandbox entry point, because the reward function evaluates final code through the
same function and would merge reward evaluation into the tool-call count. \textbf{Wording bound.}
What is measured is \textbf{parser-accepted and executed calls = 0}, \emph{not} ``the model never
attempted to call'': FC rollouts were not saved.
...
one, with nothing well-formed to censor. The mandatory instruction also \emph{degraded} the model,
final pass 31$\to$15. So we ran the probe again inside verl's own AgentLoop at 7B, under the
documented configuration, logging every generation, the parser's verdict, executions and
observations.
...
It does not retroactively change the 1.5B run, whose zero remains over-determined; we report the
two scales separately and the scale dependence is itself the finding. This probe's run terminated
with a \textbf{CUDA OOM} during the actor update following rollout collection, so no gradient step
completed; the rollout observations all precede that point.
...
signal under the observed rollout distribution, and is effectively inaccessible to policy-gradient
learning in this cold-start regime.} At 10 and 3 steps this is a \emph{mechanism} result, not an
outcome comparison, and it does not generalise to ``FC training never receives tool feedback''.
...
writing. Four obstacles were met and three dissolved on inspection (Appendix~\ref{app:norepair}). A
vLLM parser plugin does not reach the training loop at all, because verl's AgentLoop parses for
...
every one targeting the function the task asks the model to write, the same signature as Llama's
failure at a 5$\times$ smaller model, which name normalisation cannot repair because no call
carries code to normalise. A role-disambiguation few-shot aimed at that confusion made the model
\emph{more fluent at emitting the wrong call} (52$\to$64 recoverable, still 0 naming the tool) and
slightly worse at the task (15$\to$13).
...
parameter-efficient tuning, not a stack-replacement project (\S\ref{sec:future}). Accordingly
\textbf{ReAct serves as a positive-control interaction channel, not a parser-repair control}.

That leaves the sufficiency question, which we tested with the one configuration delivering tool
feedback at this scale: ReAct, 75 GRPO steps, everything else matched to the historical runs,
executing \textbf{23{,}676 tool calls}. Rescues by turn $\geq$2 over steps 0/15/30/45/60 were
\textbf{8, 6, 6, 8, 6} --- net change zero, inside the range of the zero-execution runs
($[7,9,6,6,7,6,10,8,9,8,6]$, $[8,7,6,7,7,7,7,5,6,6,8]$, $[7,5,7,5,7,8,9,9,8,9]$). Whether tool
feedback arrived made no measurable difference on this metric, and composition barely moved either:
adjacent checkpoints have solved-set Jaccard 0.974--0.983 while 85 of 542 first-turn completions
changed verbatim between steps 15 and 30 --- \textbf{the policy moves, the solved set does not}
(Appendix~\ref{sec:composition}). This does not license ``interface repair unlocks multi-turn
capability''; we tested the closest available version and it failed. It licenses the narrower
statement: \textbf{the interface determines whether tool-mediated trajectories exist at all, and
their absence is sufficient to explain why no multi-turn signal was available --- but their
presence is not sufficient to produce multi-turn learning at this scale and budget.} \textbf{This
is a ReAct arm, not a repaired-FC arm}: it changes protocol and interface together, so it cannot
separate ``a working channel is insufficient'' from ``ReAct in particular is insufficient''. It is
75 steps on one seed at 1.5B against 150-step baselines, and the step-75 evaluation was lost when
the checkpoint write exhausted the disk after training completed.
