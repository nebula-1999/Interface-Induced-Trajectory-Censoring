...
\textbf{Correction factor.} Under A2's labels --- full text, blind, \emph{no adjudication
round at all} --- the \texttt{tight} stratum's precision is 39/40 = \textbf{0.975}, so
\S5.2's headline 80/100 at 32B corrects to \textbf{$\approx$78}. A2 additionally marks 2
items \texttt{Y} that the classifier does not count, so the criterion misses calls as well as
...
\textbf{Remaining disagreements.} Fifteen items are not unanimous across A1, A2 and the
...
because the stored output hits its length ceiling, and a call wrapped in a \texttt{<tool>}
tag.
